...
用户输入格式（灵活解析）：
- 仅 URL：`https://www.youtube.com/watch?v=...`
- URL + 主题：`https://... 帮我截出有关人生启发的内容`
- URL + 详细描述：`https://... 我想要关于创业失败和重新出发的片段`

解析规则：
1. 提取 YouTube URL（必须）
2. 提取用户的**主题方向**（可选）。若用户未指定，默认选取"最精彩、最有洞察力"的片段。
...
**优先使用用户指定的主题方向**来筛选片段。例如：
- 用户说"人生启发" → 优先选择关于人生感悟、成长、选择、逆境的片段
- 用户说"创业经验" → 优先选择关于创业决策、失败教训、团队管理的片段
- 用户未指定 → 选取最精彩、最有洞察力的综合性片段

从字幕中选出 **5 条**符合主题的片段。通用选取标准：
- 金句：简洁、可引用、令人印象深刻
- 洞察：揭示关键观点或令人惊讶的事实
- 转折点：论点的重要转变
- 情感共鸣：感人或有趣
