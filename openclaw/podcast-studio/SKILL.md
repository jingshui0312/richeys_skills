...
```bash
# 使用默认设置生成脚本Prompt
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py create "人工智能的发展趋势" --check-only

# 指定风格和时长
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py create "区块链技术" --style tech --duration medium --check-only
```
...
```bash
# 从JSON文件生成音频（自动混合BGM）
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py generate-audio script.json

# 指定声音
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py generate-audio script.json --voice male-qn-qingse

# 禁用BGM（默认启用）
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py generate-audio script.json --no-bgm

# 使用自定义配置
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py generate-audio script.json --config ~/.podcast_config.json
```
...
```bash
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py list
```
...
**配置文件位置**：`/path/to/openclaw/skills/podcast-studio/config.json`

**当前配置**：
```json
{
  "bgm_file": "/path/to/openclaw/skills/podcast-studio/assets/bgm_default.mp3",
  "bgm_volume": 0.15,
  "bgm_fade_in": 2000,
  "bgm_fade_out": 3000,
...
```bash
# 生成不带 BGM 的音频
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py create "主题" --no-bgm
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py generate-audio script.json --no-bgm
```
...
1. **替换默认 BGM**：
   ...
   cp your_bgm.mp3 /path/to/openclaw/skills/podcast-studio/assets/bgm_default.mp3
   ```
...
```bash
# 跳过脚本优化，直接生成音频
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py create "主题" --no-optimize
```
...
```bash
python3 /path/to/openclaw/skills/volcengine-tos/scripts/tos_client.py configure \
...
```bash
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py create "主题" --config ~/.podcast_studio_config.json
```
...
```bash
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py create "健康生活" \
...
```bash
# 生成播客（自动进行深度分析）
python3 /path/to/openclaw/skills/podcast-studio/scripts/podcast_studio.py create "AI 代理的发展趋势" --style conversational --duration short
```
