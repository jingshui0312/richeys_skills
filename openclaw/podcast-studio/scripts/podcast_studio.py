...
class PodcastStudio:
    ...
    def _load_llm_config(self):
        ...
        env_file = Path("/root/clawd/skills/article-extractor/.env")
        if env_file.exists():
...
希望今天的分享能给你带来一些启发和思考。让我们开始吧。"""
        ...
        # 动态导入TTS
        try:
            sys.path.insert(0, '/root/clawd/skills/minimax-tts/scripts')
            import importlib.util
            spec = importlib.util.spec_from_file_location("tts", "/root/clawd/skills/minimax-tts/scripts/tts.py")
    ...
    def _upload_to_tos(self, audio_file, topic):
        ...
        try:
            # 动态导入TOS客户端
            import importlib.util
            spec = importlib.util.spec_from_file_location("tos_client", "/root/clawd/skills/volcengine-tos/scripts/tos_client.py")
            ...
            bucket = self.config.get("tos_bucket", "reports01")
