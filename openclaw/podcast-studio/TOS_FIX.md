...
**问题根因：**
在 `/root/clawd/skills/podcast-studio/scripts/podcast_studio.py` 中，TOS上传代码使用了错误的类名：
- **错误：** `TOSClient`
- **正确：** `TosClientV2`
...
**修改文件：** `/root/clawd/skills/podcast-studio/scripts/podcast_studio.py`
...
**修改文件：**
- `/root/clawd/skills/podcast-studio/scripts/podcast_studio.py`
  - 方法：`_upload_to_tos()`
  - 行数：~483-527

**未修改文件：**
- `/root/clawd/skills/podcast-studio/scripts/ai_hotspot_podcast.py`
- `/root/clawd/skills/volcengine-tos/scripts/tos_client.py`
...
### 1. 查看日志
```bash
tail -100 /root/clawd/scheduler/logs/ai-hotspot-podcast.log
```

预期输出：
```
✅ 播客生成成功
正在上传到TOS: reports01/audio/xxx.mp3
✓ 上传成功! ETag: xxx
💾 热点记录已保存
📢 播客通知
   TOS链接: https://xxx
...
- 任务脚本：`/root/clawd/skills/podcast-studio/scripts/ai_hotspot_podcast.py`
- 主脚本：`/root/clawd/skills/podcast-studio/scripts/podcast_studio.py`
- TOS客户端：`/root/clawd/skills/volcengine-tos/scripts/tos_client.py`
- Scheduler配置：`/root/clawd/scheduler/config.json`
...
2. **验证凭证有效性：**
   ```bash
   python3 /root/clawd/skills/volcengine-tos/scripts/tos_client.py list
   ```
...
1. **检查MiniMax TTS配置：**
   ```bash
   python3 /root/clawd/skills/minimax-tts/scripts/tts.py --help
   ```
