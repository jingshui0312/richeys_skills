...
- Scheduler日志：`/path/to/openclaw/scheduler/logs/scheduler.log`
...
python3 -c "
import sys
sys.path.insert(0, '/path/to/openclaw/skills/podcast-studio/scripts')
...
### 完整运行（生成播客）
```bash
python3 /path/to/openclaw/skills/podcast-studio/scripts/ai_hotspot_podcast.py
```
...
### 查看任务列表
```bash
/path/to/openclaw/scheduler/scheduler.sh list
```

### 查看任务状态
```bash
/path/to/openclaw/scheduler/scheduler.sh status
```

### 查看任务日志
```bash
/path/to/openclaw/scheduler/scheduler.sh logs ai-hotspot-podcast
```

### 手动运行任务
```bash
/path/to/openclaw/scheduler/scheduler.sh run ai-hotspot-podcast
```

### 禁用/启用任务
```bash
# 禁用
/path/to/openclaw/scheduler/scheduler.sh disable ai-hotspot-podcast

# 启用
/path/to/openclaw/scheduler/scheduler.sh enable ai-hotspot-podcast
```

### 重启Scheduler
```bash
/path/to/openclaw/scheduler/scheduler.sh restart
```
...
### Scheduler配置
位置：`/path/to/openclaw/scheduler/config.json`
...
```json
{
  "tasks": {
    "ai-hotspot-podcast": {
      ...
      "command": "python3 /path/to/openclaw/skills/podcast-studio/scripts/ai_hotspot_podcast.py",
      "timeout": 600,
      "retry": {
        "max": 2,
        "delay": 60
      ...
      "logging": {
        "file": "/path/to/openclaw/scheduler/logs/ai-hotspot-podcast.log",
        "level": "info"
      }
    }
  }
...
### 任务未执行
1. 检查Scheduler状态：`/path/to/openclaw/scheduler/scheduler.sh status`
2. 检查任务是否启用：`/path/to/openclaw/scheduler/scheduler.sh list`
3. 查看日志：`/path/to/openclaw/scheduler/scheduler.sh logs ai-hotspot-podcast`
...
如有问题，请查看：
- Scheduler框架文档：`/path/to/openclaw/SCHEDULER_FRAMEWORK.md`
- Podcast Studio文档：`/path/to/openclaw/skills/podcast-studio/SKILL.md`
- 使用指南：`/path/to/openclaw/skills/podcast-studio/USAGE_GUIDE.md`
